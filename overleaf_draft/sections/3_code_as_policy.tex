\section{Code-as-Policy: Robots that Write their Own Skills}
\label{sec:code}
...
\subsection{Zero-shot synthesis (\S3.1.1)}
\label{ssec:zeroshot}
...
\subsection{Closed-loop self-repair (\S3.1.2)}
...
\subsection{Skill-library accumulation (\S3.1.3)}
\label{ssec:skilllib}
...
ever-growing library of verified skill programs with when-to-apply guards. The key
distinction from \S3.1.2 is temporal scope: competence compounds \emph{across} tasks rather
than being rebuilt each time.

\subsection{Evolutionary program search (\S3.1.4)}
\label{ssec:search}
...
\subsection{The full self-improving loop (\S3.1.5)}
\label{ssec:fullloop}
